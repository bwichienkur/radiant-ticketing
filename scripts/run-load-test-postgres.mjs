#!/usr/bin/env node
/**
 * Starts EnhancementHub.Api against PostgreSQL, runs k6 500 VU gate profile, then stops the API.
 */
...
async function main() {
  ...
  console.log(`Starting API on ${baseUrl} with PostgreSQL...`);
  apiProcess = spawn(
    'dotnet',
    ['run', '--project', apiProject, '-c', 'Release', '--no-build', '--urls', baseUrl, '--nologo'],
    {
      ...
      env: {
        ...
        ConnectionStrings__Default: process.env.ConnectionStrings__Default
          ?? 'Host=127.0.0.1;Port=5432;Database=enhancementhub_load;Username=postgres;Password=postgres',
      },
  ...
  console.log(`API is ready. Running k6 Postgres gate profile (${profile})...`);
  ...
  const outputPath = path.join(resultsDir, `k6-postgres-${profile}-${timestamp}.txt`);
...
try {
  await main();
  console.log('k6 Postgres gate load test completed.');
} catch (error) {
