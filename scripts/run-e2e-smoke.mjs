#!/usr/bin/env node
/**
 * Starts EnhancementHub.Web, waits for readiness, runs Playwright smoke tests, then stops the server.
 */
...
async function main() {
  console.log('Building EnhancementHub.Web...');
  await run('dotnet', ['build', webProject, '-c', 'Release', '--nologo'], { cwd: repoRoot });

  console.log(`Starting Web on ${baseUrl}...`);
  webProcess = spawn(
    'dotnet',
  ...
  console.log('Web is ready. Running Playwright smoke tests...');

  await run('npx', ['playwright', 'test'], {
    cwd: e2eDir,
    env: { ...process.env, E2E_BASE_URL: baseUrl, CI: process.env.CI ?? 'true' },
  });
}
