#!/usr/bin/env node
/**
 * Starts EnhancementHub.Api, waits for readiness, runs k6 smoke load test, then stops the API.
 */
...
const k6Script = path.join(repoRoot, 'tests/load/k6-smoke.js');
...
async function main() {
  ...
  console.log('API is ready. Running k6 smoke load test...');
  ...
  const outputPath = path.join(resultsDir, `k6-smoke-${timestamp}.txt`);
...
try {
  await main();
  console.log('k6 smoke load test completed.');
} catch (error) {
